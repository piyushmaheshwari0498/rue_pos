...
class DbSaleOrder {
  ...
  Future<String> createAPIOrder(APISaleOrder order) async {
    String res = await saveAPIOrder(order);
    await DBPreferences().incrementOrderNo(order.id);
    await DbCategory().reduceAPIInventory(order.items);
    if (order.transactionId.isEmpty) {
      await DbHubManager().updateCashBalance(order.orderAmount);
    }
    await box.close();
  ...
  Future<List<SaleOrder>> getOrders() async {
    ...
    for (var key in box.keys) {
      ...
      Future.forEach(order.items, (item) async {
        ...
        Product? pro = await DbProduct().getProductDetails(product.id.toString());
        if (pro != null) {
  ...
  Future<String> saveAPIOrder(APISaleOrder order) async {
    box = await Hive.openBox<APISaleOrder>(SALE_ORDER_BOX);
  ...
  String _getOrderKey(SaleOrder order) {
   ...
   String _getAPIOrderKey(APISaleOrder order) {
    // return "${order.date}_${order.time}";
    return "${order.tracsactionDateTime.millisecondsSinceEpoch}";
  }
