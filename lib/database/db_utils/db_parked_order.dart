...
class DbParkedOrder {
  ...
  Future<List<APIParkOrder>> getAPIOrders() async {
    // LazyBox box = await Hive.openLazyBox(PARKED_ORDER_BOX);
    box = await Hive.openBox<APIParkOrder>(PARKED_ORDER_BOX);

    List<APIParkOrder> list = [];

    for (var key in box.keys) {
      var item = await box.get(key);
      var order = item as APIParkOrder;

      Future.forEach(order.items, (item) async {
        var product = item as OrderItem;
        APIOrderItem? pro = await DbOrderItem().getAPIProductDetails(product.id);
        if (pro != null) {
          product.productImage = pro.productImage;
        }
      });
  ...
  Future<String> saveAPIOrder(APIParkOrder order) async {
    box = await Hive.openBox<APIParkOrder>(API_PARKED_ORDER_BOX);
    String orderKey = _getAPIOrderKey(order);
    await box.put(orderKey, order);
    // await box.close();
    return orderKey;
  }
  ...
  String _getAPIOrderKey(APIParkOrder order) {
    // return "${order.date}_${order.time}";
    return "${order.transactionDateTime.millisecondsSinceEpoch}";
  }
  ...
  Future<bool> deleteOrder(ParkOrder order) async {
   ...
   Future<bool> deleteAPIOrder(APIParkOrder? order) async {
    box = await Hive.openBox<APIParkOrder>(API_PARKED_ORDER_BOX);
    String orderKey = _getAPIOrderKey(order!);
    APIParkOrder parkOrder = box.get(orderKey);
    parkOrder.delete();
    return true;
  }
